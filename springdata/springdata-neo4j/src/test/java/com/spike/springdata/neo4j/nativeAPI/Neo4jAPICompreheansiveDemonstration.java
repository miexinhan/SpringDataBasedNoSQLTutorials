...
public class Neo4jAPICompreheansiveDemonstration {
  ...
  /**
   * clean
   * @param dbDirPath The path to delete
   */
   ...
   * @param gds The {@link GraphDatabaseService}
   */
  ...
  static void navigateUsingLables(final GraphDatabaseService gds) {
    ...
    try (Transaction tx = gds.beginTx();) {
      ...
      ResourceIterable<Node> users =
          GlobalGraphOperations.at(gds).getAllNodesWithLabel(NodeTypeEnum.USERS);
      for (Node user : users) {
  ...
  static void traverseUsingCoreJavaAPI(final GraphDatabaseService gds) {
    ...
    try (Transaction tx = gds.beginTx();) {
      ...
      Iterable<Relationship> allHasSeenRelathinshipOfUser1 =
          user1.getRelationships(DynamicRelationshipType.withName(RelTypeEnum.HAS_SEEN.name()));
      ...
      for (Node friend : friendsOfUser1) {
        for (Relationship relationship : friend.getRelationships(Direction.OUTGOING,
          RelTypeEnum.HAS_SEEN)) {
  ...
  static void traverseUsingBuiltinTraverseAPI(final GraphDatabaseService gds) {
    ...
    try (Transaction tx = gds.beginTx();) {
      ...
      TraversalDescription travesalDesc =
          gds.traversalDescription().relationships(RelTypeEnum.IS_FRIEND_OF)
              .relationships(RelTypeEnum.HAS_SEEN, Direction.OUTGOING)
      ...
      logger
          .error("Something strange happened when traverse using built-in traverse API, refer", e);
  ...
  /**
   * Traverse using Neo4j Built-in Traverse API: {@link TraversalDescription} and customed
   * {@link Executor}
   * @param gds The {@link GraphDatabaseService}
   */
    ...
    try (Transaction tx = gds.beginTx();) {
      ...
      TraversalDescription travesalDesc =
          gds.traversalDescription().relationships(RelTypeEnum.IS_FRIEND_OF)
              .relationships(RelTypeEnum.HAS_SEEN, Direction.OUTGOING)
              .uniqueness(Uniqueness.NODE_GLOBAL).evaluator(Evaluators.atDepth(2))
              .evaluator(new CustomedEvaluator(user1));
   ...
   * (1) determine whether current visited node should add to the traversal result<br/>
   * (2) determine next steps: (a) continue futher down the path, (b) abandon current path, move to
