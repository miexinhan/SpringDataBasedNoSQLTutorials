	...
	private static final void traversal(GraphDatabaseService gds, Node rootNode,
		...
		try (Transaction tx = gds.beginTx();) {
			...
			if (TraversalAlgorithmEnum.DEPTH_FIRST.equals(traversalAlgorithmEnum)) {
				traversalDescription = gds.traversalDescription().relationships(RelTypeEnum.CHILD, Direction.OUTGOING)
						.depthFirst();
			...
			logger.error("Something strange happened when traversal data in depth-first style,  refer", e);
		}
	}
