...
public class Neo4jCypherAPIDemonstration {
  ...
  static void populateGraphData(GraphDatabaseService gds, ExecutionEngine engine) {
    ...
    try (BufferedReader reader =
        new BufferedReader(new FileReader(new File("cypher/user-and-movie-schema.cypher")));) {
      String line = null;
    ...
    try (Transaction tx = gds.beginTx()) {
      ...
      for (int i = 0; i < 5; i++) {
        ...
        if (i < 3) {// user
          Neo4jAppUtils.createNodeIndex(gds, nodeIndexName, node, PropEnum.NAME.name()
              .toLowerCase(), node.getProperty(PropEnum.NAME.name().toLowerCase()));
          Neo4jAppUtils.createNodeIndex(gds, nodeIndexName, node, PropEnum.EMAIL.name()
              .toLowerCase(), node.getProperty(PropEnum.EMAIL.name().toLowerCase()));
          Neo4jAppUtils.createNodeIndex(gds, nodeIndexName, node,
            PropEnum.AGE.name().toLowerCase(), node.getProperty(PropEnum.AGE.name().toLowerCase()));
