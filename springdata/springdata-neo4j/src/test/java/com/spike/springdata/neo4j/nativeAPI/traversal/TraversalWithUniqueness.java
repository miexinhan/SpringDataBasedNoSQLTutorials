...
public class TraversalWithUniqueness {
	...
	private static void traversal(GraphDatabaseService gds, final Node startNode, final Node endNode,
		...
		try (Transaction tx = gds.beginTx();) {
			TraversalDescription traversalDescription = gds.traversalDescription().relationships(RelTypeEnum.KNOWNS)
					.evaluator(new Evaluator() {
						...
						public Evaluation evaluate(Path path) {
							...
							// endNode);
							Path singlePath = GraphAlgoFactory.shortestPath(pathFinder, 1).findSinglePath(currentNode,
									endNode);
