...
public class TraversalWithBidirection {
	...
	public static void main(String[] args) {
		...
		/**
		 * Question: does Jane(0) and Ben(4) can know each other through
		 * transitive KNOWS relationship<br/>
		 * 
	...
	private static void traversal(GraphDatabaseService gds, Node startNode1, Node startNode2) {
		try (Transaction tx = gds.beginTx();) {
			...
			TraversalDescription startSideDescription = gds.traversalDescription().relationships(RelTypeEnum.KNOWNS)
					.uniqueness(Uniqueness.NODE_PATH);

			TraversalDescription endSideDescription = gds.traversalDescription().relationships(RelTypeEnum.KNOWNS)
					.uniqueness(Uniqueness.NODE_PATH);

			BidirectionalTraversalDescription biTraversalDescription = gds.bidirectionalTraversalDescription()
					.startSide(startSideDescription).endSide(endSideDescription)
					// .startSide(
