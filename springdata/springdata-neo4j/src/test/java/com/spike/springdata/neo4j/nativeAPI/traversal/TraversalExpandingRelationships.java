...
public class TraversalExpandingRelationships {
	...
	static void customedExpander(GraphDatabaseService gds, Node startNode) {
		...
		mappings.put(0, Arrays.asList(new RelationshipType[] { RelTypeEnum.IS_FRIEND_OF, RelTypeEnum.WORK_WITH }));
	...
	static void orderingRelationshipForExpand(GraphDatabaseService gds, Node startNode) {
		...
		try (Transaction tx = gds.beginTx();) {
			...
			PathExpander<?> pathExpander = new OrderedByTypeExpander().add(RelTypeEnum.IS_FRIEND_OF)
					.add(RelTypeEnum.WORK_WITH).add(RelTypeEnum.LIKES);
	...
	static void standardExpander(GraphDatabaseService gds, Node startNode) {
		try (Transaction tx = gds.beginTx();) {
			...
			PathExpander<?> pathExpander = StandardExpander.DEFAULT.add(RelTypeEnum.WORK_WITH)
					.add(RelTypeEnum.IS_FRIEND_OF).add(RelTypeEnum.LIKES);
