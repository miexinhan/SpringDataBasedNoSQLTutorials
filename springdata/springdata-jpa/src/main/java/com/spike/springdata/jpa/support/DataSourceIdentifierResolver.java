...
public final class DataSourceIdentifierResolver {
	...
	public List<DataSourceIdentifier> resolve(String propertyFileBasename) {
		...
		Map<String, DataSourceIdentifier> identifierDataSourceIdentifierMap = new HashMap<String, DataSourceIdentifier>();
		for (String key : keys) {
			if (key.startsWith(PropertyKey.MULTIPLE_TARGET_PREFIX.getKey())) {
				...
				DataSourceIdentifier dataSourceIdentifier = identifierDataSourceIdentifierMap.get(identifier);
